...
export function SearchBar() {
  ...
  function removeRecent(shortId: number) {
    const next = loadRecent().filter(r => r.short_id !== shortId)
      ...
      {query === '' && recent.length > 0 && (
        <div className="mt-3">
          ...
          <div className="flex flex-wrap gap-2">
            {recent.map((r) => (
              // A button can't nest another button, so the chip is a div holding the
              // navigate button and the remove cross side by side.
              <div
                key={r.short_id}
              ...
              >
                ...
                <span className="w-px self-stretch my-1.5 bg-zinc-700/50" />
                <button
                  onClick={() => removeRecent(r.short_id)}
                  aria-label={`Remove ${r.fighter_id} from recent searches`}
                  className="px-2 py-2 text-zinc-500 hover:text-zinc-200 transition-colors cursor-pointer"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
